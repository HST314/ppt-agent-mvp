...
import { badge, button, element, field, formatTime, metadataList, shortHash, versionTimeline } from "../components/index.js?v=2026.08.23.100340566066";
import { actionMessage, codeBlock, draftGuard, parseSlideIds, runAction, section, stageGrid } from "./shared.js?v=2026.08.23.100340566066";

export async function render(context) {
  const view = context.authoritativeView || await api.planning(context.taskId, context.controller);
...
function planningStage(view, kind, context) {
  ...
  confirm.addEventListener("click", async () => {
    ...
    const result = await runAction({ buttonNode: confirm, region: confirmMessage, busyLabel: "正在确认…", action: () => action(context.taskId), success: "当前版本已确认。" }).catch(() => null);
    if (result) context.goTo(kind === "narrative" ? "outline" : "sample");
  });
