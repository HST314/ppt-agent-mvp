...
**降级保底**:motion.min.js 以经典脚本(UMD,`window.Motion`)本地加载,`file://` 直接可用;如果 Motion 缺失,脚本会强制把所有 `data-anim` 元素设为 `opacity:1`,内容永远可读。不要改成远程 CDN 脚本或 ES 模块导入(交付离线校验拒绝外链,`file://` 下模块会被 CORS 拦截)。
