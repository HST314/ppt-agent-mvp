...
DELIVERY_PAGE=re.compile(r"^/tasks/([^/]+)/delivery$")
...
class App:
    ...
    def __call__(self,environ,start_response):
        ...
        try:
            ...
            if method=="GET" and path=="/": return self.home_page(start_response)
            ...
            m=DELIVERY_PAGE.match(path)
            if method=="GET" and m:return self.delivery_page(start_response,m.group(1))
    ...
    def home_page(self,start):
        raw='''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>PPT Agent</title><style>body{font:16px system-ui;max-width:760px;margin:64px auto;padding:0 24px;color:#172033}form{border:1px solid #ccd3df;border-radius:12px;padding:24px}label,input,select,button{display:block;width:100%;box-sizing:border-box;margin:10px 0;padding:10px}output{display:block;color:#b42318;min-height:24px}</style><h1>PPT Agent 桌面工作区</h1><p>创建任务后，从任务卡开始完成规划、样品、全稿、检查与交付。</p><form id="create"><label>任务 ID<input id="taskId" required pattern="[A-Za-z0-9_-]+"></label><label>模式<select id="mode"><option value="manual">manual</option><option value="auto">auto</option></select></label><button type="submit">创建任务</button><output id="result" aria-live="polite"></output></form><script>document.getElementById('create').onsubmit=async e=>{e.preventDefault();const id=document.getElementById('taskId').value.trim(),r=await fetch('/v1/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({task_id:id,mode:document.getElementById('mode').value})});if(r.ok)location.href='/tasks/'+encodeURIComponent(id);else document.getElementById('result').textContent=((await r.json()).error||{}).message||'创建失败';};</script></html>'''.encode()
...
<h1>大纲工作区</h1><nav><a href="/tasks/{esc(task_id)}">任务/资料</a><strong>大纲</strong><a href="/tasks/{esc(task_id)}/samples">样品</a></nav><p>每次操作形成可见版本，历史版本支持非破坏回退。</p>
...
<script>const ID={json.dumps(task_id)};document.querySelectorAll('form[data-kind]').forEach(f=>{{const k=f.dataset.kind,send=async(url,data)=>{{const r=await fetch(url,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(data)}});if(r.ok)location.reload();else f.querySelector('output').textContent=((await r.json()).error||{{}}).message||'操作失败';}};f.onsubmit=e=>{{e.preventDefault();send('/v1/tasks/'+ID+'/'+k,{{markdown:f.querySelector('textarea').value}})}};f.querySelector('[name=generate]').onclick=()=>send('/v1/tasks/'+ID+'/'+k+'/generate',{{prompt:f.querySelector('[name=prompt]').value}});f.querySelector('[name=confirm]').onclick=()=>send('/v1/tasks/'+ID+'/'+k+'/confirm',{{}});}});document.querySelectorAll('.rollback').forEach(b=>b.onclick=()=>fetch('/v1/tasks/'+ID+'/planning/rollback',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify({{kind:b.dataset.kind,hash:b.dataset.hash}})}}).then(()=>location.reload()));</script></html>''').encode()
    ...
    def delivery_page(self,start,task_id):
        view=self.service.delivery_view(task_id); esc=self.esc; deck=self.service.deck_view(task_id).get("deck") or {}; delivery=view.get("latest") or {}
        delivered=bool(delivery); delivery_hash=delivery.get("hash","")
        raw=(f'''<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>交付与派生</title><style>body{{font:16px system-ui;max-width:900px;margin:32px auto;padding:0 24px;color:#172033}}section{{border:1px solid #ccd3df;border-radius:12px;padding:20px;margin:16px 0}}label,textarea,button{{display:block;width:100%;box-sizing:border-box;margin:10px 0;padding:10px}}output{{display:block;color:#b42318;min-height:24px}}</style><h1>交付与派生</h1><p id="deliveryStatus">交付状态：{'已确认' if delivered else '待用户确认'}</p><section><h2>最终交付</h2><p>当前候选：<code>{esc(deck.get('hash',''))}</code></p><button id="confirmDelivery" {'disabled' if delivered else ''}>确认最终交付</button></section><section><h2>交付后派生</h2><label>派生要求<textarea id="derivePrompt" placeholder="输入新一轮修改要求"></textarea></label><button id="derive" {'disabled' if not delivered else ''}>从已交付版本派生</button></section><output id="result" aria-live="polite"></output><script>const ID={json.dumps(task_id)},DECK={json.dumps(deck.get('hash',''))},DELIVERY={json.dumps(delivery_hash)},out=document.getElementById('result');async function send(path,body){{const r=await fetch('/v1/tasks/'+ID+'/delivery/'+path,{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(body)}}),d=await r.json();if(r.ok)location.reload();else out.textContent=(d.error||{{}}).message||'操作失败'}}document.getElementById('confirmDelivery').onclick=()=>send('confirm',{{deck_hash:DECK,actor:'user'}});document.getElementById('derive').onclick=()=>send('derive',{{delivery_hash:DELIVERY,prompt:document.getElementById('derivePrompt').value}});</script></html>''').encode()
        start("200 OK",[("Content-Type","text/html; charset=utf-8"),("Content-Length",str(len(raw)))]); return [raw]
