...
状态机以 `TaskState(stage, status, mode)` 为唯一事实源。`stage` 表示业务阶段，`status` 表示正交运行状态。样品确认、终稿冻结和最终离线写包必须由 `actor=user` 执行；auto 也不能绕过。生成全稿、检查通过、终稿冻结或文件存在均不产生 `completed`，只有已冻结终稿的离线包原子发布并校验成功后才完成。
