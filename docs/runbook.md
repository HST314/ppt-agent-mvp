...
样品 Job 的结构化结果包含页面 `section` 片段、`DesignIntent` 和共享 CSS 资产；确认事件会冻结三者。全稿 Job 只生成未确认页面，并必须复用确认事件中的设计意图和共享资产。`PresentationTechnicalContract` 只包含画布、页面标识、资源、安全和交付边界；若替换 Skill 后视觉语言仍未改变，优先检查 Agent 是否读取了新快照，以及全稿是否错误复用了其他任务的确认上下文，不要在 assembler 中增加风格分支。
