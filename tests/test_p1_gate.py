 ...
 def test_openapi_and_preview_bypass_is_absent(self):
  ...
  for path,item in spec["paths"].items():
   for operation in item.values():
    ...
    if path not in {"/healthz","/livez","/readyz"}: self.assertTrue(any(v.get("$ref")=="#/components/responses/Error" for k,v in responses.items() if not str(k).startswith("2")),path)
  with tempfile.TemporaryDirectory() as p:
