...
class AC13RoundLimitE2E(test_ac_12_modes.AC12ModesE2E):
    def test_auto_limit_waits_for_human_without_false_completion(self):
        ...
        self.assertFalse(result["report"]["passed"]); self.assertTrue(result["delivery_allowed"])
        ...
        for token in ("修复轮次","发布前 TechnicalGate","Agent 修复","当前候选预览","定位","确定终稿"): self.assertIn(token,module)
