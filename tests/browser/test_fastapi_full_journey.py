...
class FastAPIFullJourneyGate(unittest.TestCase):
    ...
    def test_create_to_delivery_and_derive_in_one_shell(self):
        ...
        resource_dialog = page.get_by_role("dialog", name="没有检测到图片资源")
        resource_dialog.wait_for()
        self.assertIn("资源为空不是错误", resource_dialog.get_by_role("paragraph").first.text_content())
        resource_dialog.get_by_role("button", name="继续无图片").click()
